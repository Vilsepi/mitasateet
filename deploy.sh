...
REGION="eu-west-1"
PROFILE="heap"
BUCKET="mitasateet.heap.fi"

SYNC_BASE_ARGS="s3://$BUCKET/ --profile $PROFILE --region $REGION"
SYNC_FILE_ARGS="--exclude '*' --include '*.html' --include 'assets/*'"

if [ $# -lt 1 ]
then
  echo "Simulating S3 sync, no changes are made"
  eval aws s3 sync . $SYNC_BASE_ARGS $SYNC_FILE_ARGS --dryrun
  echo "To deploy, re-execute with --doit"
else
  if [ $1 == "--doit" ]; then
    echo "Uploading assets"
    eval aws s3 sync . $SYNC_BASE_ARGS $SYNC_FILE_ARGS
  else
